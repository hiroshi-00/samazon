  ...
  scope :sort_list, -> { 
    {
      ...
      "売上の安い順" => "price asc",
      "売上の高い順" => "price desc", 
      "売上の古い順" => "updated_at asc", 
      "売上の新しい順" => "updated_at desc"
    }
  }
  ...
  def self.get_monthly_billings
    ...
    billings.each_with_index do |b, i|
      ...
      if hash[b.updated_at.strftime("%Y-%m")][:price_daily].present?
        hash[b.updated_at.strftime("%Y-%m")][:billing_date] = hash[b.updated_at.strftime("%Y-%m")][:billing_date] = b.updated_at.strftime("%Y-%m")
  ...
  def self.get_daily_billings
    ...
    billings.each_with_index do |b,i|
      ...
      if hash[b.updated_at.to_date.to_s][:price_daily].present?
        hash[b.updated_at.to_date.to_s][:billing_date] = hash[b.updated_at.to_date.to_s][:billing_date] = b.updated_at.strftime("%Y-%m")
        ...
        hash[b.updated_at.to_date.to_s][:billing_date] = b.updated_at.strftime("%Y-%m")
