class Dashboard::ProductsController < ApplicationController
  ...
  def index
    ...
    if params[:keyword].present?
      ...
      @products = Product.search_for_id_and_name(keyword).sort_order(sort_query).display_list(params[:page])
    else
      keyword = ""
      @total_count = Product.count
      @products = Product.sort_order(sort_query).display_list(params[:page])
    end
